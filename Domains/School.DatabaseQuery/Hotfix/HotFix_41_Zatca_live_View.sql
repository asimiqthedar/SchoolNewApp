...
,ParentName=coalesce(par.ParentName ,s.ParentName)      
 ,FatherIQAMA=ISNULL(par.FatherIqamaNo,ISNULL(parentIqama.IqamaNumber,'') )      
,Nationality=isnull(par.Nationality,s.Nationality)  
 ...
 -- where s.Status='Posted'     
  ...
  on tis.InvoiceNo=s.InvoiceNo      
	...
	(  
		select   
			InvoiceNo ,IqamaNumber ,ParentName,ParentCode  
		from  
		(  
			select      
				ROW_NUMBER() over(partition by tis.InvoiceNo order by  tis.InvoiceNo desc) as RN              
				,tis.InvoiceNo         
				,tis.IqamaNumber
				,tis.ParentName
				,tis.ParentCode
			from ALS_LIVE.DBO.INV_InvoiceDetail tis  
		)t  
		where t.RN=1  
	) parentIqama  
	on tis.InvoiceNo=s.InvoiceNo 
