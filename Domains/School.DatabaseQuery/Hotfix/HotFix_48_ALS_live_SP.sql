...
BEGIN                     
 ...
 select   
   ROW_NUMBER() over( order by SOPNumber)  ,    
    SOPNumber,SOPType,DocID,DocDate,CustomerNumber,ItemNumber,Quantity,UnitPrice=(UnitPrice),IntegrationStatus,Error      
...
begin    
 ...
 select @MaxSeqNumber= max(SeqNum) from INT_SalesDistributionSourceTable where SOPNumber=@invoiceno     
