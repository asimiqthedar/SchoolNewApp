DROP PROCEDURE IF EXISTS sp_deleteparentStudentRecord
GO

CREATE PROCEDURE sp_deleteparentStudentRecord
@ParentCode nvarchar(50)=''
,@StudentCode nvarchar(50)
as
begin
	declare @AllowParentDelete int=0
	--Check for Student--Delete Stduent
	if exists(select 1 from tblStudent where StudentCode=@StudentCode)
	begin
		declare @parentid int=0
		declare @studentid int=0
		select @studentid=studentid, @parentid =parentid  from tblStudent where StudentCode=@StudentCode

		select 'Start: Total Parent Student Record'
		select * from tblStudent where  parentid=@parentid
		select 'End: Total Parent Student Record'
	
		select 'Start: Analysing Student record'
		select * 
		into #InvoiceListStudent  
		from INV_InvoiceDetail 
		where ParentId=@parentid  and StudentId=@studentid

		select * from tblStudent where StudentCode=@StudentCode
		select * from tblParent where @parentid =parentid
		select * from tblFeeStatement where ParentId= @parentid and StudentId=@studentid

		select * from INV_InvoicePayment where InvoiceNo in (select InvoiceNo from #InvoiceListStudent)
		select * from INV_InvoiceDetail where InvoiceNo in (select InvoiceNo from #InvoiceListStudent)
		select * from INV_InvoiceSummary where InvoiceNo in (select InvoiceNo from #InvoiceListStudent)

		select 'Start: Deleting student and reference record'			
		
		delete from tblFeeStatement where ParentId= @parentid and StudentId=@studentid

		delete from INV_InvoicePayment where InvoiceNo in (select InvoiceNo from #InvoiceListStudent)
		delete from INV_InvoiceDetail where InvoiceNo in (select InvoiceNo from #InvoiceListStudent)
		delete from INV_InvoiceSummary where InvoiceNo in (select InvoiceNo from #InvoiceListStudent)

		delete from tblStudent where StudentCode=@StudentCode

		if exists(select 1 from tblStudent where  @parentid =parentid and StudentCode!=@StudentCode)
		begin
			select 'Check: Unable to delete parent as other student also mapped with this parent'
			
		end
		else
		begin
			delete from tblParent where @parentid =parentid
			set @AllowParentDelete=1
		end

		select 'End: Deleting student and reference record'
		select 'End: Analysing Student record'
	end

	
	------parent process
	if @AllowParentDelete=1 OR exists(select 1 from tblParent where  ParentCode =@ParentCode )
	begin
		declare @parentidForParent int=0
		select @parentidForParent =parentid  from tblParent where ParentCode =@ParentCode

		select 'Start: Total Parent Student Record'
		select @parentidForParent
		select * from tblStudent where  parentid =@parentidForParent
		select 'End: Total Parent Student Record'

		if exists(select 1 from tblStudent where  parentid =@parentidForParent)
		begin
			select 'Parent is mapped with student, Please delete student first'			
		end
		else
		begin
			select 'Start: Analysing parent record'
			select * from tblFeeStatement where ParentId= @parentidForParent
			select * into #InvoiceListParent  from INV_InvoiceDetail where ParentId=@parentidForParent  OR ParentCode =@ParentCode

			select * from INV_InvoicePayment where InvoiceNo in (select InvoiceNo from #InvoiceListParent)
			select * from INV_InvoiceDetail where InvoiceNo in (select InvoiceNo from #InvoiceListParent)
			select * from INV_InvoiceSummary where InvoiceNo in (select InvoiceNo from #InvoiceListParent)

			select 'Start: Deleting student and reference record'
			
			
			delete from tblFeeStatement where ParentId= @parentidForParent
			...
			select 'End: Deleting student and reference record'
			select 'End: Analysing parent record'
		end

	end
	else
	begin
		select 'All Set: Clear all record'
	end
end
GO
...
begin         
 ...
 truncate table [tblStudentOpenApplyProcessed]        
	...
	select country, substring(country,1,3),1,0,CAST(getdate() AS DateTime),1
	from 
	(
	  select distinct country from OpenApplyParents where country is not null
	  union
	  select distinct country from OpenApplyStudents where country is not null
	)t
	where ltrim(rtrim(country)) not in (select ltrim(rtrim(CountryName)) from tblCountryMaster)
...
begin                                
 ...
 (                                
  select                             
   ...
   , NewValueJson=       
   (      
    SELECT distinct jc.StudentCode,jc.StudentName,jc.StudentArabicName,jc.StudentEmail,jc.IqamaNo,jc.Mobile,CountryName=cm.CountryName       
    FROM [tblStudentOpenApplyProcessed] jc 
	left join tblCountryMaster cm on jc.NationalityId=cm.CountryId
	where jc.[StudentCode] = os.[StudentCode] FOR JSON PATH, WITHOUT_ARRAY_WRAPPER       
   )      
   , OldValueJson=       
   (      
    SELECT distinct jc.StudentCode,jc.StudentName,jc.StudentArabicName,jc.StudentEmail,jc.IqamaNo,jc.Mobile,CountryName=cm.CountryName       
    FROM [tblStudent] jc 
	left join tblCountryMaster cm on jc.NationalityId=cm.CountryId
	where jc.[StudentCode] = stu.[StudentCode] FOR JSON PATH,WITHOUT_ARRAY_WRAPPER       
   )      
   ,NameUpdated=CASE                           
   ...
   ,IqamaNoUpdated=CASE WHEN ltrim(rtrim(isnull( os.IqamaNo ,''))) COLLATE SQL_Latin1_General_CP1256_CI_AS<>ltrim(rtrim(isnull( stu.IqamaNo ,'')))        
    THEN 1 ELSE 0 END    
	,CountryUpdated=CASE WHEN os.NationalityId<>stu.NationalityId
    THEN 1 ELSE 0 END    
 ...
 WHERE t.NameUpdated=1 OR t.ArabicNameUpdated=1 OR t.EmailUpdated=1 OR t.IqamaNoUpdated=1 OR t.CountryUpdated=1     
  ...
  SELECT         
   ...
   (      
    SELECT distinct      
	 ...
	 ,FatherCountryName=fm.CountryName ,MotherCountryName=mm.CountryName
    FROM tblParentOpenApplyProcessed jc
	left join tblCountryMaster fm on jc.FatherNationalityId=fm.CountryId
	left join tblCountryMaster mm on jc.MotherNationalityId=mm.CountryId
	where jc.[ParentCode] COLLATE SQL_Latin1_General_CP1256_CI_AS= os.[ParentCode] FOR JSON PATH, WITHOUT_ARRAY_WRAPPER       
   )      
   ,OldValueJson=       
   (      
    SELECT distinct      
	...
	left join tblCountryMaster fm on jc.FatherNationalityId=fm.CountryId
	left join tblCountryMaster mm on jc.MotherNationalityId=mm.CountryId
	where jc.ParentCode = stu.ParentCode FOR JSON PATH,WITHOUT_ARRAY_WRAPPER       
   )      
...
begin      
	...
	SELECT        
	ISNULL(SUM(CASE WHEN ind.InvoiceType = 'Entrance Fee' THEN isnull(ind.TaxableAmount,0)  ELSE 0 END),0) AS MonthlyEntranceRevenue,       
	ISNULL(SUM(CASE WHEN ind.InvoiceType = 'Uniform Fee' THEN isnull(ind.TaxableAmount,0)  ELSE 0 END),0) AS MonthlyUniformRevenue,       
	ISNULL(SUM(CASE WHEN ind.InvoiceType = 'Tuition Fee' THEN isnull(ind.TaxableAmount,0)  ELSE 0 END),0) AS MonthlyTuitionRevenue      
