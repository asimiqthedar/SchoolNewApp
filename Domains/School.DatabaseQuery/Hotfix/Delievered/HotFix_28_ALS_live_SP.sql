...
DROP PROCEDURE IF EXISTS  [dbo].[sp_ProcessOpenApplyRecordToTable]
GO
...
begin     
 ...
 select                            
  ...
  -- ,GradeId=1--isnull(gm.GradeId,1)    
  --,CostCenterId=1                            
  --,SchoolId=1                            
  --,SectionId=1--ISNULL( ts.SectionId,1)      
  --,NationalityId=999999--isnull(tc.CountryId,999999)    --Update filed when new chnages done                
  -- ,StudentStatusId =1             
  ...
  select    
    StudentId=0 ,StudentCode=student_id ,StudentImage='' ,ParentId=0 ,StudentName ,StudentArabicName ,StudentEmail ,DOB     
 ...
 ,WithdrawDate ,WithdrawAt=null ,WithdrawYear ,Fees=null ,IsGPIntegration=0 ,TermId=0 ,AdmissionYear ,PrinceAccount     
 ,IsActive=1 ,IsDeleted=0 ,UpdateDate=getdate() ,UpdateBy=1 ,p_id_school_parent_id    
 ...
 from tblStudent where StudentCode in (select StudentCode from #studentOpenApply)    
  ...
  --Fine all student which are new    
  INSERT INTO [tblStudentOpenApplyProcessed]    
  (    
 StudentId,StudentCode,StudentImage,ParentId,StudentName,StudentArabicName,StudentEmail,DOB,IqamaNo    
 ,NationalityId,GenderId,AdmissionDate,GradeId,CostCenterId,SectionId,PassportNo,PassportExpiry,Mobile    
 ...
 union     
 select p_id_school_parent_id from #studentOpenApply    
  )    
 ...
 ParentId=0,  ParentCode ,ParentImage ,FatherName ,FatherArabicName ,FatherNationalityId ,FatherMobile ,FatherEmail ,IsFatherStaff ,FatherIqamaNo     
   ...
   ,IsActive=1 ,IsDeleted=0 ,UpdateDate=getdate() ,UpdateBy=1     
  from  #FinalParenToProcess t1    
  where ParentCode not in (select ParentCode from tblParent)    
...
DROP PROCEDURE IF EXISTS  [dbo].sp_RejectOpenApplyStudent
GO

CREATE PROCEDURE sp_RejectOpenApplyStudent  
@LoginUserId int=0  
...
DROP PROCEDURE IF EXISTS  [dbo].sp_ApproveOpenApplyParent
GO

CREATE PROCEDURE sp_ApproveOpenApplyParent  
@LoginUserId int=0  
,@NotificationGroupDetailId int  
as  
begin  
...
DROP PROCEDURE IF EXISTS  [dbo].sp_RejectOpenApplyParent
GO

CREATE PROCEDURE sp_RejectOpenApplyParent  
@LoginUserId int=0  
,@NotificationGroupDetailId int  
as  
begin  
...
DROP PROCEDURE IF EXISTS  [dbo].sp_ApproveOpenApplyStudent
GO

CREATE PROCEDURE sp_ApproveOpenApplyStudent  
@LoginUserId int=0  
,@NotificationGroupDetailId int  
as  
begin  
  ...
  --------------Start: Student record          
   ...
   join tblNotificationTypeMaster tgm on tg.NotificationTypeId=tgm.NotificationTypeId  
   join tblNotification noti on tgd.TableRecordId=noti.NotificationId  
   join [tblStudentOpenApplyProcessed] stu on noti.RecordId=stu.studentCode  
...
DROP PROCEDURE IF EXISTS  [dbo].[sp_GetNotificationOpenApplyStudent]
GO
...
DROP PROCEDURE IF EXISTS  [dbo].[sp_GetNotificationOpenApplyParent]
GO
 ...
 BEGIN TRY  
  ...
  (                          
   select                       
    ...
    , NewValueJson= (SELECT * FROM [tblStudentOpenApplyProcessed] jc where jc.[StudentCode] = os.[StudentCode] FOR JSON PATH, WITHOUT_ARRAY_WRAPPER )                    
  ...
  (              
   SELECT   
    ...
    ,NameUpdated=CASE                 
     WHEN ltrim(rtrim(isnull( os.MotherName ,''))) COLLATE SQL_Latin1_General_CP1256_CI_AS<>ltrim(rtrim(isnull( stu.MotherName ,'')))  
     THEN 1 ELSE 0 END                
    ,EmailUpdated=CASE                 
     when ltrim(rtrim(isnull( os.MotherEmail ,''))) COLLATE SQL_Latin1_General_CP1256_CI_AS<>ltrim(rtrim(isnull( stu.MotherEmail ,'')))                          
     THEN 1 ELSE 0 END                  
