//var page = new WebPage();
//var system = require("system");
//var phoneImg =  'data:image/png;base64, '; // Pass the image path as a command-line argument
//var faxImg =  'data:image/png;base64,'; // Pass the image path as a command-line argument
//var schoolImg = 'data:image/png;base64,'; // Pass the image path as a command-line argument
//page.paperSize = {
//	format: "Letter",
//	orientation: "portrait",
//	footer: {
//		height: "3.7cm",
//		contents: phantom.callback(function (pageNum, numPages) {
//			//return "<div style='text-align:center;'><small>Page " + pageNum + " of " + numPages + "</small></div>";
//			return "<div style='width:100%; text-align:center;'>" +
//				"<table style='width:100%; border-collapse:collapse;'>" +
//				"<tr>" +
//				"<td colspan='3' style='border-bottom:1px solid #ddd; text-align:center;font-size:12px;'>" +
//				"<h3 style='color:#00ADEF;'>If you have any queries please contact Enas Aldajani on 920051888 Ext:110 & ealdajani@alsschools.com</h3>" +
//				"</td>" +
//				"</tr>" +
//				"<tr>" +
//				"<td colspan='3' style='border-bottom:1px solid #ddd; text-align:center;font-size:12px;'>" +
//				"<h3 style='color:#00ADEF;'>Advance Learning Co. Ltd. An Nakheel, Riyadh, PO BOX 221985 Riyadh 11311 Kingdom of Saudi Arabia</h3>" +
//				"<h3>شركة التعليم المتطور المحدودة  حي النخيل الغربي, ص.ب 221985, الرياض  11311, المملكة العربية السعودية</h3>" +
//				"</td>" +
//				"</tr>" +
//				"<tr>" +
//				"<td style='width:33.33%; text-align:left;'>" +
//				"<img src='" + phoneImg + "' alt='Phone' style='width:16px; height:16px;'>" +
//				"<span>Telephone:966 11 2070926</span>" +
//				"</td>" +
//				"<td style='width:33.33%; text-align:center;'>" +
//				"<img src='" + faxImg +"' alt='Fax' style='width:16px; height:16px;'>" +
//				"<span>Fax:966 11 2070927</span>" +
//				"</td>" +
//				"<td style='width:33.33%; text-align:right;'>" +
//				"<img src='" + schoolImg +"' alt='Web' style='width:16px; height:16px; margin-right:5px;'>" +
//				"<span>web:www.alsschools.com</span>" +
//				"</td>" +
//				"</tr>" +
//				"<tr>" +
//				"<td colspan='3' style='text-align:center;'>" +
//				"<small>" +
//				"Page " + pageNum +
//				" of " + numPages +
//				"</small>" +
//				"</td>" +
//				"</tr>" +
//				"</table>" +
//				"</div>";
//		})
//	},
//	header: {
//		height: "0cm",
//		contents: phantom.callback(function (pageNum, numPages) {
//			var html = system.args[4].replace("{headerMid:", "").replace("}", "");
//			if (pageNum == 1) {
//				html = system.args[3].replace("{headerTop:", "").replace("}", "") + html;
//			}
//			return html;
//		})
//	}
//};

//// assume the file is local, so we don't handle status errors
//page.open(system.args[1], function (status) {
//	// export to target (can be PNG, JPG or PDF!)
//	page.render(system.args[2]);
//	phantom.exit();
//});
var page = require('webpage').create();
var system = require('system');

// Base64-encoded images
var phoneImg = 'data:image/png;base64,';
var faxImg = 'data:image/jpeg;base64,';
var schoolImg = 'data:image/png;base64,';

// Configure the page
page.paperSize = {
	format: 'Letter',
	orientation: 'portrait',
	footer: {
		height: '3.5cm',
		contents: phantom.callback(function (pageNum, numPages) {
			// Only render footer if it's the last page
			if (pageNum === numPages) {

				var bankDetail =
					"<div style='width:100%; margin-top:6px;'>" +
					"<table border='0' cellpadding='0' cellspacing='0' bordercolor='grey' style='font-size:14px;width:100%;border-color: grey;border-width: thin;'>" +
						"<tr>" +
							"<td colspan=6 valign='top' align='left'>" +
								"<b style='font-size: 14px; padding: 3px;'>BANK DETAILS</b>" +
							"</td>" +
						"</tr>" +

						"<tr>" +
							"<td valign='top' align='left' style='border: 1px solid grey; padding: 3px;'>Account Name</td>" +
							"<td valign='top' align='left' style='border: 1px solid grey; padding: 3px;'>Bank</td>" +
							"<td valign='top' align='left' style='border: 1px solid grey; padding: 3px;'>Branch</td>" +
							"<td valign='top' align='left' style='border: 1px solid grey; padding: 3px;'>Account No</td>" +
							"<td valign='top' align='left' style='border: 1px solid grey; padding: 3px;'>Swift Code</td>" +
							"<td valign='top' align='left' style='border: 1px solid grey; padding: 3px;'>IBAN</td>" +
						"</tr>"+

						"<tr>" +
							"<td valign='top' align='left' style='border: 1px solid grey; padding: 3px;'>Advance Learning Company Ltd</td>" +
							"<td valign='top' align='left' style='border: 1px solid grey; padding: 3px;'>EMIRATES NBD</td>" +
							"<td valign='top' align='left' style='border: 1px solid grey; padding: 3px;'>Riyadh, Saudi Arabia</td>" +
							"<td valign='top' align='left' style='border: 1px solid grey; padding: 3px;'>1016046166901</td>" +
							"<td valign='top' align='left' style='border: 1px solid grey; padding: 3px;'>EBILAEADXXX</td>" +
							"<td valign='top' align='left' style='border: 1px solid grey; padding: 3px;'>SA6095000001016046166901</td>" +
						"</tr>" +
					"</table>" +
					"</div>";

                var footer = "<div style='width:100%; text-align:center;'>" +

					"<table border='0' cellpadding='0' cellspacing='0' bordercolor='grey' style='font-size:14px;width:100%;border-color: grey;border-width: thin;'>" +
						"<tr>" +
						"<td colspan=9 valign='top' align='left'>" +
							"<h3 style='color: #B58B00; padding-left: 3px;'>* Sibling discount is valid till 15 December.</h3>" +
							"<h3 style='color: #B58B00; padding-left: 3px; '>* Amount is VAT Excluded </h3>" +
						"</td>" +
						"</tr>" +					
					"</table>" +

					"<table border='0' cellpadding='0' cellspacing='0' style='width:100%; border-collapse:collapse;'>" +

					"<tr>" +
					"<td colspan='3' style='border-bottom:1px solid #ddd; text-align:center;font-size:12px;'>" +
					"<h3 style='color:#00ADEF;'>If you have any queries please contact Enas Aldajani on 920051888 Ext:110 & ealdajani@alsschools.com</h3>" +
					"</td>" +
					"</tr>" +

					"<tr>" +
					"<td colspan='3' style='text-align:center;font-size:12px;'>" +
					"<h3 style='color:#00ADEF;'>Advance Learning Co. Ltd. An Nakheel, Riyadh, PO BOX 221985 Riyadh 11311 Kingdom of Saudi Arabia</h3>" +
					"<h3>شركة التعليم المتطور المحدودة  حي النخيل الغربي, ص.ب 221985, الرياض  11311, المملكة العربية السعودية</h3>" +
					"</td>" +
					"</tr>" +

					"<tr>" +
					"<td style='width:33.33%; text-align:left;' valign='top'>" +
					"&nbsp;&nbsp;&nbsp;<img src='" + phoneImg + "' alt='Phone' style='width:16px; height:16px;'>" +
					"<span> 920051888</span>" +
					"</td>" +
					"<td style='width:33.33%; text-align:center;' valign='top'>" +
					"<img src='" + faxImg + "' alt='Fax' style='width:16px; height:16px;'>" +
					"<span> info@alsschools.com</span>" +
					"</td>" +
					"<td style='width:33.33%; text-align:right;' valign='top'>" +
					"<img src='" + schoolImg + "' alt='Web' style='width:16px; height:16px; margin-right:5px;'>" +
					"<span>www.alsschools.com</span>&nbsp;&nbsp;&nbsp;" +
					"</td>" +
					"</tr>" +
					"</table>" +
					"</div>";

				return bankDetail + footer;
			} else {
				return ""; // Return empty content for other pages
			}
		})
	},
	header: {
		height: '0cm',
		contents: phantom.callback(function (pageNum, numPages) {
			var html = system.args[4].replace("{headerMid:", "").replace("}", "");
			if (pageNum == 1) {
				html = system.args[3].replace("{headerTop:", "").replace("}", "") + html;
			}
			return html;
		})
	}
};


// Open the page and render it to PDF or image
page.open(system.args[1], function (status) {
	if (status === 'success') {
		page.render(system.args[2]);
	} else {
		console.log('Failed to open the page.');
	}
	phantom.exit();
});
